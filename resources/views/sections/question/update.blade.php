          ...
          <div class="card-body">
            ...
            <form action="{{ route('question.update',$question->id) }}"  method="POST" enctype="multipart/form-data">
                ...
                <div class="col-sm-10">
                  <select name="category_id" class="form-select" aria-label="Default select example">
                    @foreach ($categories as $category){
                      <option value="{{ $category['id'] }}" {{ $question->category_id == $category['id']? 'selected' :'' }}   >{{ $category['title_uz'] }}</option>
                    }
                    @endforeach
                  </select>
                </div>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <input type="text" required  name="question" value="{{ $question->question }}" class="form-control">
                </div>
              </div>
              <div class="row mb-3">
                <label for="inputEmail" class="col-sm-2 col-form-label">Javob</label>
                <div class="col-sm-10">
                  <input type="text" required  name="response" value="{{ $question->response }}" class="form-control">
                </div>
              </div>
